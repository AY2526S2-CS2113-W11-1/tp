...
* [Features](#features)
    ...
    * [Checking monthly fee of student: `fee-summary`](#calculating-monthly-student-fees-fee-summary)
    * [View monthly income: `monthly-income`](#monthly-income-report-monthly-income)
...
Calculates the total fees generated by a specific student for a given month.

Format: `fee-summary INDEX ym/YYYY-MM`

- The `INDEX` refers to the number shown in the active student list (use `list` to view it).
- The `ym/` prefix is mandatory, followed by the month in `YYYY-MM` format.
- The command calculates total revenue based on the student's assigned lessons and their fee-per-lesson rate for the specified month.

Examples of usage:

- `list` followed by `fee-summary 1 ym/2026-04` — Calculates and displays total fees for the 1st student in the active list for April 2026.

Expected behaviour:
- Displays a report containing the student's name, the target month, and the total fee that month.
- If the `INDEX` is invalid or the month format is incorrect, an error message is shown.

---

### Monthly income report: `monthly-income`

Aggregates the total revenue generated across all active students for a specific month.

Format: `monthly-income ym/YYYY-MM`

- The `ym/` prefix is mandatory, followed by the month in `YYYY-MM` format.
- This command provides a high-level overview of total monthly earnings.

Example of usage:

- `monthly-income ym/2026-04` — Displays the total revenue across all active students for April 2026.

Expected behaviour:
- Displays the Monthly Income Report, showing the target month and the grand total revenue.
- If the month format is incorrect, an error message is displayed: `Invalid format. Use ym/YYYY-MM (e.g. ym/2026-03)`.
